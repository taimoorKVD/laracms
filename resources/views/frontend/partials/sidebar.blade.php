<div class="col-md-4 col-xl-3">
    <div class="sidebar px-4 py-md-0">
        ...
        <div class="row link-color-default fs-14 lh-24">
            ...
            @if($categories->count() > 0)
                @foreach ($categories as $category)
                    <div class="col-6"><a href="{{ route('web.category', $category->id) }}">{{ $category->name }}</a></div>
                @endforeach
            @else
                <div class="col-12"><a href="#">No categories found.</a></div>
            @endif
        </div>
        ...
        <div class="gap-multiline-items-1">
            @if($tags->count() > 0)
                @foreach ($tags as $tag)
                    <a class="badge badge-secondary" href="{{ route('web.tag', $tag->id) }}">{{ $tag->name }}</a>
                @endforeach
            @else
                <a class="badge badge-secondary" href="#">No tag found.</a>
            @endif
